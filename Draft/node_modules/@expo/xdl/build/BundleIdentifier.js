...
async function configureBundleIdentifierAsync(projectRoot, exp) {
  ...
  if (bundleIdentifierFromPbxproj && bundleIdentifierFromConfig) {
    if (bundleIdentifierFromPbxproj === bundleIdentifierFromConfig) {
      ...
      (0, _ProjectUtils().logWarning)(projectRoot, 'expo', `We detected that your Xcode project is configured with a different bundle identifier than the one defined in ${configDescription}.`);
      ...
      if (!hasBundleIdentifierInStaticConfig) {
        (0, _ProjectUtils().logInfo)(projectRoot, 'expo', `If you choose the one defined in ${configDescription} we'll automatically configure your Xcode project with it.
...
async function updateAppJsonConfigAsync(projectDir, exp, newBundleIdentifier) {
  var _rawStaticConfig$expo;

  const paths = (0, _config().getConfigFilePaths)(projectDir);
...
async function hasBundleIdentifierInStaticConfigAsync(projectDir, exp) {
...
//# sourceMappingURL=__sourcemaps__/BundleIdentifier.js.map
